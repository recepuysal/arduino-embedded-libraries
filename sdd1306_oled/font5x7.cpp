...
namespace {
// Adafruit GFX classic 5x7 font table (first 256 chars).
static const uint8_t kAdafruitFont[] = {
...
static const uint8_t kGlyphUnknown[5] = {0x7F, 0x49, 0x49, 0x49, 0x36};
static const uint8_t kGlyphDegree[5] = {0x06, 0x09, 0x09, 0x06, 0x00};
}  // namespace

namespace Font5x7 {
const uint8_t* Lookup(uint16_t cp) {
  if (cp == 0x00B0U) {
    return kGlyphDegree;
  }
  if (cp <= 255U) {
    return &kAdafruitFont[static_cast<uint16_t>(cp) * 5U];
  }
  return kGlyphUnknown;
}

uint8_t GlyphWidth() { return 5U; }
uint8_t GlyphHeight() { return 8U; }
uint8_t GlyphAdvance() { return 6U; }
}  // namespace Font5x7
